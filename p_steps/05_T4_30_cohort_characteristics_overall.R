...
if (TEST){
  testname <- "test_D5_Table_1_overall"
...
for (i in drug_names) {
  ...
  data <- readRDS(file = paste0(thisdirinput, "/D3_incidence_con_caratterizzazione_", i, ".rds"))
  
  data <- as.data.table(data)
  
  assign(paste0("D3_incidence_con_caratterizzazione_", i), data)
...
for (j in drug_names) {
  ...
  D5_nocov <- get(paste0("D3_incidence_con_caratterizzazione_", j))[, .(
                N          = .N,
  ...
  for (i in covariates_binary_tab1) {
    ...
    tmp <- get(paste0("D3_incidence_con_caratterizzazione_", j))[, .(
                N = .N,
                tmp_N = sum(get(i)==1),
    ...
    if (is.null(D5_cov)) {
      ...
      D5_cov <- merge(D5_cov, tmp, by = c("period", "N"))
    }
    
  }
  
  # create the final D5 by merging the previous two
  D5 <- merge(D5_nocov, D5_cov, by = c("period", "N"), all = F)
...
for (j in drug_names) {
  ...
  saveRDS(get(paste0("D5_", j)), file = paste0(thisdiroutput, "/D5_Table_1_caratterizzazione_coorte_", j, "_overall.rds"))
  write.csv(get(paste0("D5_", j)), file = paste0(thisdiroutput, "/D5_Table_1_caratterizzazione_coorte_", j, "_overall.csv"))
