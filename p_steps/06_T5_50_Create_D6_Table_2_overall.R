...
if (TEST){ 
  testname <- "test_D6_Table_2_overall"
...
for (j in drug_names) {
  ...
  D5 <- read.csv(paste0(thisdirinput, "D5_Table_2_caratterizzazione_coorte_sottopopolazione_CAP_", j, "_overall.csv"))
  
  D5 <- as.data.table(D5)
  
  assign(paste0("D5_Table_2_caratterizzazione_coorte_sottopopolazione_CAP_",j,"_overall"), D5)
...
for (k in drug_names) {
  ...
  tab_nice <- copy(get(paste0("D5_Table_2_caratterizzazione_coorte_sottopopolazione_CAP_", k, "_overall")))
  ...
  setorder(tab_nice, rownum, period)
  ...
  pre_cols   <- grep("^pre", data_cols, value = TRUE)
  nota_cols   <- grep("^nota", data_cols, value = TRUE)
  ...
  outputfile <- tab_nice
  nameoutput <- "D6_Table_2_caratterizzazione_coorte_sottopopolazione_CAP"
  assign(nameoutput, outputfile)
  
  # rds
  saveRDS(outputfile, file = file.path(thisdiroutput, paste0(nameoutput,"_", k,"_overall.rds")))
  # csv
