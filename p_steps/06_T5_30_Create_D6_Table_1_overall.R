...
if (TEST){ 
  testname <- "test_D6_Table_1_overall"
...
for (j in drug_names) {
  ...
  D5 <- read.csv(paste0(thisdirinput, "D5_Table_1_caratterizzazione_coorte_", j, "_overall.csv"))
  
  D5 <- as.data.table(D5)
  
  assign(paste0("D5_Table_1_caratterizzazione_coorte_",j,"_overall"), D5)
...
for (k in drug_names) {
  ...
  tab_nice <- copy(get(paste0("D5_Table_1_caratterizzazione_coorte_", k, "_overall")))
